...
from sderl.soc.networks_jax import FeedForwardNN
...
class SOCAgent:
    ...
    def __init__(self, sampler, hidden_size=256, actor_learning_rate=1e-2,
        ...
        self.actor_optimizer = self.actor_op_init.init(self.actor.flat_params)
    ...
    def save_params_jax(self, file_name):
        pass

    def load_params_jax(self, file_name):
        pass
    ...
    def get_action_scale(self, state, do_scale=False):
        ...
        #return self.actor.forward(params, state)
        return vmap(self.actor.forward, in_axes=(None, 0), out_axes=0)(params, state)

    def get_action(self, state):

        #return self.actor.forward(self.actor.params, state)
        return self.actor.forward(self.actor.flat_params, state)

    def get_action_batch(self, states):

        #return vmap(self.actor.forward, in_axes=(None, 0), out_axes=0)(params, states)
        return vmap(self.actor.forward, in_axes=(None, 0), out_axes=0)(self.actor.flat_params, states)

    def get_grad_action(self, state):

        #return jacfwd(self.actor.forward, argnums=0)(self.actor.params, state)
        return jacfwd(self.actor.forward, argnums=0)(self.actor.flat_params, state)

    def get_grad_action_batch(self, states):

        grad_actions = vmap(
            jacfwd(self.actor.forward, argnums=0),
            #jacrev(self.actor.forward, argnums=0),
            in_axes=(None, 0),
            out_axes=0,
        )(self.actor.flat_params, states)
        return grad_actions

    def update_jax(self, grads):
        """ upate parameter for the actor network
        """
        updates, opt_state = self.actor_op_init.update(grads, self.actor_optimizer)

        self.actor.flat_params = optax.apply_updates(self.actor.flat_params, updates)
    ...
    def train_batch_jax(self, max_n_ep=100, max_n_steps='1e8'):
        ...
        # save initialization
        file_path = os.path.join(model_dir_path, self.log_name + '_soc-actor-start.npy')
        with open(file_path, 'wb') as f:
            jnp.save(f, self.actor.flat_params, allow_pickle=True)
        ...
        for i_episode in range(max_n_ep):
            ...
            # initialize running cost, det and stoch integrals
            phi_t = jnp.zeros(batch_size)
            phi_fht = jnp.empty(batch_size)
            det_int_t = jnp.zeros((batch_size, self.actor.flat_size))
            det_int_fht = jnp.empty((batch_size, self.actor.flat_size))
            stoch_int_t = jnp.zeros((batch_size, self.actor.flat_size))
            stoch_int_fht = jnp.empty((batch_size, self.actor.flat_size))
            ...
            while not done:
                ...
                # get action
                actions = self.get_action_batch(states)
                grad_actions = self.get_grad_action_batch(states)

                # get new state
                new_states, rewards, done, obs = sampler.step(actions)

                # get used Brownian increment
                dbt = obs[0]
                ...
                # update deterministic integral
                det_int_t = det_int_t + vmap(jnp.matmul, (0, 0), 0)(actions, grad_actions)

                # update stochastic integral
                stoch_int_t = stoch_int_t + vmap(jnp.matmul, (0, 0), 0)(dbt, grad_actions)
                ...
                if idx.shape[0] != 0:
                    ...
                    # save running costs and running integrals for the arrived trajectorries
                    phi_fht = phi_fht.at[idx].add(phi_t[idx])
                    det_int_fht = det_int_fht.at[idx].add(det_int_t[idx])
                    stoch_int_fht = stoch_int_fht.at[idx].add(stoch_int_t[idx])
            ...
            # compute cost functional (loss)
            self.loss = jnp.mean(phi_fht)
            self.var_loss = jnp.var(phi_fht)

            # compute gradient
            self.grad_loss = jnp.mean(
                det_int_fht + jnp.expand_dims(phi_t, axis=1) * stoch_int_fht
            )
            ...
            # update networks
            self.update_jax(self.grad_loss)
            ...
            if success or i_episode + 1 == max_n_ep or max_len:
                ...
                file_path = os.path.join(model_dir_path, self.log_name + '_soc-actor-last.npy')
                with open(file_path, 'wb') as f:
                    jnp.save(f, self.actor.flat_params, allow_pickle=True)
    ...
    def train_batch(self, max_n_ep=100, max_n_steps='1e8'):
        ...
        for i_episode in range(max_n_ep):
            ...
            # compute effective loss
            #self.eff_loss = torch.mean(0.5 * det_int_fht + phi_fht.detach() * stoch_int_fht)
            self.eff_loss = torch.mean(0.5 * det_int_fht)
            #self.eff_loss = torch.mean(0.5 * det_int_fht + phi_fht.detach() * stoch_int_fht)
